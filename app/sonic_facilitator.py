"""Sonic/DAG Facilitator for Vennatta Core x402 payments."""
...
class SonicFacilitator:
    """Sonic/DAG payment facilitator for x402 protocol.

    Handles USDC and native S token payments on Sonic (EVM-compatible).
    """
    ...
    def __init__(
        ...
        """Initialize Sonic facilitator.

        Args:
            rpc_url: Sonic RPC endpoint
            supported_networks: List of supported network IDs (e.g., eip155:146 for Sonic)
        """
        self.rpc_url = rpc_url
        self.supported_networks = supported_networks or [
            "eip155:146",  # Sonic mainnet
        ]
        logger.info(f"⚡⚡⚡ SonicFacilitator initialized with RPC: {rpc_url}")
    ...
    def verify_payment(
        ...
        """
        try:
            logger.info("⚡⚡⚡ Sonic verify_payment() called ⚡⚡⚡")
            ...
            logger.info(f"  ✅ Sonic payment VERIFIED: {payer}, {amount}")
            ...
            logger.error(f"❌❌❌ Sonic verification EXCEPTION: {e} ❌❌❌")
            import traceback
    ...
    def _verify_signature_cryptographic(
        ...
        """Verify Sonic signature cryptographically.
    ...
    def settle(
        ...
        Returns:
            Settlement result
        """
        logger.info("⚠️ Sonic settle() called")
        try:
            payer = payload.get("payload", {}).get("authorization", {}).get("payer", "")
            amount = requirements.get("amount", "0")

            logger.info(f"⚠️ Sonic settlement: {payer}, {amount}")
            ...
            return {
                ...
                "network": requirements.get("network", "eip155:146"),
                "amount": amount,
            }
        except Exception as e:
            logger.error(f"Sonic settlement error: {e}")
            return {
                "success": False,
...
if __name__ == "__main__":
    facilitator = SonicFacilitator()

    # Test payment
    payload = {
        "payload": {
            "authorization": {"payer": "0x1234567890123456789012345678901234567890"},
